...
import { getMostRecentUnreadMessages, markMessageAsRead } from "../services/supabase/inbox";
...
function DropdownNotifications({ align }) {
  ...
  const { data: notifications, isLoading } = useQuery({
    queryKey: ["inbox"],
    queryFn: async () => await getMostRecentUnreadMessages(),
  });

  const { mutate: mutateNotification } = useMutation({
    mutationKey: ["inbox"],
    mutationFn: async (id) => await markMessageAsRead(id),
    onSuccess: () => {
  ...
  // Mark message as read when accessing it
  ...
  return (
    <div className="relative inline-flex">
      <button
        disabled={isLoading}
        ref={trigger}
        className={`w-8 h-8 disabled:cursor-progress flex items-center justify-center hover:bg-gray-100 lg:hover:bg-gray-200 dark:hover:bg-gray-700/50 dark:lg:hover:bg-gray-800 rounded-full ${
      ...
      >
        ...
        {notifications?.length && notifications?.find((item) => !item.is_read) && (
            ...
            {notifications?.map((message) => (
              <li className="border-b border-gray-200 dark:border-gray-700/60 last:border-0">
                <ReadMore message={message}>
                  <button
                    className="text-start border-none outline-none block py-2 px-4 hover:bg-gray-50 dark:hover:bg-gray-700/20 cursor-pointer"
                    to="#0"
                    handleClick={() => handleRead(message.id)}
                  >
                    <span className="block text-sm mb-2">
                      📧 <span className="font-medium text-gray-800 dark:text-gray-100">{message?.fullname}</span>{" "}
                      {message.message?.length > 84 ? `${message?.message?.slice(0, 84)}...` : message?.message}
                    </span>
                    <span className="block text-xs font-medium text-gray-400 dark:text-gray-500">
                      {format(message?.created_at, "LLL dd, yyyy")}
                    </span>
                  </button>
                </ReadMore>
              </li>

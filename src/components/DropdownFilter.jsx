...
function DropdownFilter({ headings, setHeadings, align }) {
        ...
        <div ref={dropdown}>
              ...
              <li key={index} className="py-1 px-3">
                <label className="flex items-center">
                  <input
                    ...
                    onChange={() =>
                      setHeadings(
                        headings.map((item) => (item.label === col.label ? { ...item, show: !col.show } : item))
                      )
                    }
                  />
                  <span className="text-sm font-medium ml-2">{col.label}</span>
                </label>
              </li>
            ))}
          </ul>
          <div className="py-2 px-3 border-t border-gray-200 dark:border-gray-700/60 bg-gray-50 dark:bg-gray-700/20">
            <ul className="flex items-center justify-between">
              <li>
                <button
                  onClick={() => setHeadings(headings.map((item) => ({ ...item, show: true })))}
